...
ggplot(sales, aes(x = WeekStart, y = Sales)) +
  ...
  ggtitle("Weekly Restaurant Sales over time") +
...
ggplot(sales, aes(x = WeekNum, y = Sales, col = factor(Year))) +
  geom_point(size = 2.5, shape = 21, fill = "grey") + geom_line() +
  geom_smooth( size = 1.25, se = FALSE, span = 0.35) +
  ...
  ggtitle("Weekly Restaurant Sales, Week over Week ") +
